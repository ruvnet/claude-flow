...
grep -qE "spawnCliSync|spawnNpxSync|spawnSync" "$F" || miss="$miss no-safe-spawn"
...
[[ -x "$F" ]] || miss="$miss not-executable"
node --check "$F" 2>/dev/null || miss="$miss syntax-error"
grep -qE "spawnCliSync|spawnNpxSync|spawnSync|_sessions\.mjs" "$F" || miss="$miss no-safe-exec"
