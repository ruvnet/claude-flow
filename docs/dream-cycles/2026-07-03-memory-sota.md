...
1. **Implement AutoMem-style memory RL training loop in AgentDB** (ADR-167): add a two-phase meta-learning system — Phase 1 optimizes the memory scaffold (what/how to store) from trajectory review; Phase 2 trains a dedicated `memory-specialist` agent from successful episodes. Target: ≥2x improvement on `@claude-flow/performance benchmark --suite memory-long-horizon`. This is the single highest-leverage memory enhancement identified in 2026 SOTA.
