...
> Given SONA's `distillLearning()` DISTILL step, when the confidence-forgetting gate is switched from `EWCConsolidator.getPenalty(oldWeights=[oldConf], newWeights=[newConf])` (silently reads only `globalFisher[0]`, 1 of 384 dimensions, identical for every pattern) to the pre-existing `computeConfidencePenalty()` (averages the full 384-dim Fisher diagonal) paired with `updateFisherFromConfidences()` (replacing an ad-hoc per-pattern `recordGradient` loop with the batch method built for this exact call site), then EWC's forgetting-penalty should become a function of each pattern's own embedding-derived importance rather than an identical constant for every pattern, subject to: (1) existing tests remain green; (2) `distillLearning()`'s public return shape and callers stay compatible; (3) $0 evaluation cost; (4) the fix changes only how the forgetting-penalty gate discriminates among patterns, not which patterns get distilled.
...
| Gist SHA-256 (pre-witness content) | `d566520ebd1c48054af45c3cf69b5dad4e91a4c5dbcbe3cb23d54e462b6b2bce` |
| Witness stamp | `89946458fe8fe16297950ada976d3bcc6185763e1a9d46a06ae5af4661d13d67` |
