...
| 2026-09-03 | memory | SmartRetrieval's MMR diversity step used token-Jaccard text overlap instead of embedding-cosine (already computed upstream, discarded before reaching MMR) — matches Qdrant/Weaviate's 2025-2026 shipped pattern; automation scan shipped `dream-cycle-backlog-guard.yml` (3rd-time-flagged, first-time-acted-on finding) | #TBD | #TBD | yes | ACCEPT | discriminating test: baseline picks lexical-diverse-but-semantic-near-duplicate paraphrase second, candidate picks the genuinely different topic second; 460/461 full-suite green (1 pre-existing) | TBD | 08-24..09-02 (9 nights) all still open/draft, none merged; automation backlog-guard workflow shipped tonight to make this visible without re-diagnosis |
