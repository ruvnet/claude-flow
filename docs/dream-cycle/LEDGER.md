...
| 2026-08-24 | swarm | `QueenCoordinator.weightedConsensus()` trust weights computed but structurally could not reach the vote tally (rode inertly in an opaque value blob) — threaded through propose()/vote() into raft/byzantine/gossip tally logic, [0,1]-clamped | #3085 | #3086 | yes | ACCEPT | gossip weighted-flip test: baseline `approved:false` → candidate `approved:true` (approvalRate≈0.905); 226/226 tests | 08d44c58... | 08-14..08-19: 4/6 merged (08-16/17/18/19), 2 correctly-unmerged REJECT (08-14/15) |
| 2026-08-25 | performance | `HNSWIndex.Quantizer.productQuantize()/productQuantizeDistance()` correct but never called by the search path (dispatch fixed, PQ-vector normalization skipped) | #3093 | #3094 | yes | ACCEPT | recall@10: 0.097 (pre-fix) → 0.270 (post-fix) on seeded synthetic corpus; 461/462 tests (1 pre-existing unrelated env failure) | 300da158... | 08-24 still OPEN awaiting review |
| 2026-08-26 | security | `authorizeMcpTool()` trusted unsigned `CLAUDE_FLOW_PRINCIPAL_ID` env var for caller identity (ASI07) — ADR-377's built-but-unwired Ed25519 `verifyInvocationToken` bound into the live chokepoint | #3102 | #3103 | yes | ACCEPT-with-caveats | 6/6 new deterministic auth scenarios pass post-fix, fail pre-fix; `@claude-flow/security` 583/583, `@claude-flow/codex` 241/241 | fd944eae... | 08-24/25 still OPEN awaiting review |
| 2026-08-27 | intelligence | `distillLearning()`'s EWC forgetting-gate called `getPenalty([oldConf],[newConf])`, collapsing to 1 of 384 Fisher dims for every pattern — `computeConfidencePenalty`/`updateFisherFromConfidences` (already implemented, docstring'd for this exact call site, 0 prior callers) wired in | #3109 | #3110 | yes | ACCEPT-with-caveats | baseline: new regression test fails (methods never called); candidate: passes. 152/152 tests (+4 new), tsc clean | 89946458... | 08-24/25/26 still OPEN awaiting review; 08-20..23 gap unresolved |
