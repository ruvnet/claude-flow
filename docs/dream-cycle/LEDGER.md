...
| 2026-08-30 | performance | `diskann-backend.ts` (375 lines, zero call sites, `@ruvector/diskann` not a dependency) removed; `claudemd-generator.ts`'s generated CLAUDE.md corrected from the unreproduced "150x-12,500x"/DiskANN claim to point at the reproducible `benchmark-intelligence.mjs --only=hnsw` command instead of a hardcoded multiplier; also ported #3104's pnpm-lockfile fix (v3-side CI) | #3129 | #3130 | yes | ACCEPT | baseline-fails/candidate-passes test confirmed via git-stash; full suite byte-identical 106-failure list baseline vs candidate (pre-existing env gap); live re-run of benchmark-intelligence.mjs found recall@10 0.9867(N=5k)/0.9233(N=20k) — itself disagreeing with the "~0.99" doc figure, reported not papered over; reviewer REJECTed v1 (CI red, 11-row ledger backfill, ambiguous cited number) — all 3 addressed same night | 6f56440a... | one-row-per-run rule restored after v1 mistakenly backfilled 08-20..29 (11 rows) into this branch; that ledger-repair work deferred to its own effort, not bundled here |
