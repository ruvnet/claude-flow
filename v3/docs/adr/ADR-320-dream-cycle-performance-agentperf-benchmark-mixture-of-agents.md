# ADR-320: Agentic Inference Benchmarking Standard + Mixture-of-Agents Test-Time Scaling
...
- ADR-320 collision: multiple prior dream branches have created ADR-320 with different content. This ADR uses 320 because it is the next available number on `main` (ADR-319 is the last merged). Human reviewer must reconcile or renumber when merging dream branches.
