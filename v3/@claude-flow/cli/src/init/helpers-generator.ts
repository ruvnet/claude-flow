...
export function generateHookHandler(): string {
  ...
  const lines = [
    ...
    // 'compact-auto' handlers below — all three fire the identical
    // consolidate+persist sequence, just on different triggering events.
    // Removes the separate chained session-end hook PreCompact used to fire.
    'function endSession() {',
    ...
    '  },',
    '',
    // ADR-320: these used to be followed by a separately-chained session-end
    // hook in settings.json (2 spawns per PreCompact fire). Calling
    // endSession() inline here removes that spawn without changing what
